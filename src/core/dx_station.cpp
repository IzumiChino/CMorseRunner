...
DxStation::DxStation() {
    ...
    hisCall = ini.call;
    rst     = 599;
    wpm     = static_cast<int>(std::round(
        ini.wpm * 0.5f * (1.0f + static_cast<float>(std::rand()) / RAND_MAX)));
...
void DxStation::processEvent(StationEvent ev) {
    if (oper.state == OperState::Done || oper.state == OperState::Failed) return;
    ...
    switch (ev) {
        ...
        case StationEvent::MeFinished:
            ...
            if (state == StationState::Sending) break;  // we're busy transmitting
            if (myStation_) {
                oper.receivedMsg(myStation_->msg, myStation_->hisCall);
            }
            if (oper.state == OperState::Done || oper.state == OperState::Failed) break;
