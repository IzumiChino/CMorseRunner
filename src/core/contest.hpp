...
class Contest
{
	...
	void userSentCq();
	void userSentCqText(const std::string &text);
	// hisCall: the call typed in the UI entry box
	void userSentExchange(const std::string &hisCall, int sentNr);
	void userSentExchangeText(const std::string &text,
		const std::string &hisCall,
		int sentNr);
	void userSentTuAndLog(
		const std::string &hisCall, int copiedRst, int copiedNr);
	void userSentTuAndLogText(const std::string &tuText,
	...
	void abortCurrentTransmit();
	void userSentText(const std::string &text, const MsgSet &markers);
	...
	Qso makeQsoLogEntry(
		const std::string &hisCall, int copiedRst, int copiedNr);
	void configureNewDx(DxStation *dx) const;
	void spawnStations();
	void flushMacroQueue();
	// monBlk: normalized self-monitor envelope (may be empty when not sending)
	// monGain: linear volume from the monitor slider
	TSingleArray mixAndProcess(
